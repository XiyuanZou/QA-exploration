...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default="outputs/doc_reconstruction/qwen-2.5-14b/trial_and_error_prompt_method_10trials", help='Address for output results file')
    parser.add_argument('--exploration_model_name', default="deepseek-chat", type=str) #gpt-4o-mini, deepseek-chat, Qwen/Qwen2.5-14B-Instruct
    ...
    parser.add_argument('--exploration_budget', type=int, default=50, help='how many actions can be taken before performing the final task')
    ...
    #load prompts
    question_generation_prompt_template = load_prompt("prompts/doc_reconstruction/question_generation_prompt_with_trial_and_error.txt")
    ...
    #summarization_prompt_template = load_prompt("prompts/doc_reconstruction/summarization_prompt.txt")
    ...
    for sample in tqdm(dataset):
        ...
        for i in range(args.exploration_budget):
            if qa_history == "":
                question_generation_prompt = question_generation_prompt_template.format(state=cur_state, qa_history="None") #qa_history="None" #state consists of known content & past QA history
            else:
                question_generation_prompt = question_generation_prompt_template.format(state=cur_state, qa_history=qa_history) #qa_history=qa_history
